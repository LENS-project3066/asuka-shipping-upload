...
/**
 * ⑥ **進捗イベントが 1 度も来ない端末で、送れているのに abort しない**（2026-09-07 夕）。
 *
 * 黒鎺さんの iPhone は「📤 送信中…」のままサーバー側に痕跡が 1 件も残らなかった。
 * 初版は送信前に **STALL_TIMEOUT_MS(30 秒)** で武装し、張り直しを上りの progress イベント
 * だけに頼っていたので、**進捗を報告しない端末では実際に送れていても必ず 30 秒で abort**
 * していた（＝「無進捗」のつもりで「**無イベント**」を殺していた）。
 *
 * ⚠⚠ ここは**実時間を待たずに**「何秒で武装したか」で見る ── 実際に 30 秒待つテストは
 *   書けないし、書いても遅くて誰も回さない。⛔ 静的な文字列照合に戻さないこと
 *   （旧テストは progress を購読していると「書いてあるか」しか見ておらず、この穴を通した）。
 */
test('⑥ 進捗が来ていない間は長い枠で待つ（無イベントを無進捗と誤診しない）', async () => {
  const h = await submitOne({ imageSupport: true, xhr: true });
  const SHORT = 30000, LONG = 120000;
  const short = h.timerWaits.filter(ms => ms === SHORT).length;
  const long = h.timerWaits.filter(ms => ms === LONG).length;
  // 送信前の武装は必ず長い方。進捗が来てから初めて短い方へ切り替わる。
  assert.ok(long > 0,
    '送信前に長い枠(' + LONG + 'ms)を張っていない = 進捗を報告しない端末を 30 秒で殺す形に戻っている');
  assert.ok(short > 0,
